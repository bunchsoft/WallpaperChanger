...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func setupMenu() {
        ...
        // Refresh Now option
        menu.addItem(NSMenuItem(title: "Refresh Wallpaper Now", action: #selector(refreshWallpaper), keyEquivalent: "r"))
        ...
        let picsumItem = NSMenuItem(title: "Random (from picsum.photos)", action: #selector(setSource(_:)), keyEquivalent: "")
        ...
        let staticUrlItem = NSMenuItem(title: "Static URL...", action: #selector(setStaticUrl), keyEquivalent: "")
        if case .staticUrl = currentSource {
            staticUrlItem.state = .on
        }
        sourceMenu.addItem(staticUrlItem)
        
        let jsonApiItem = NSMenuItem(title: "JSON API...", action: #selector(setJsonApi), keyEquivalent: "")
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    alert.icon = NSImage(systemSymbolName: "exclamationmark.triangle", accessibilityDescription: "Error")
        ...
        case .jsonApi(let apiUrlString, let jsonPath):
            ...
            for component in pathComponents {
                if let dict = currentValue as? [String: Any], let value = dict[component] {
                    currentValue = value
                } else if let array = currentValue as? [Any], let index = Int(component), index < array.count {
    ...
    func downloadAndSetWallpaper(from url: URL) async throws {
        ...
        let tempFileURL = FileManager.default.temporaryDirectory.appendingPathComponent("wallpaper-\(UUID().uuidString).jpg")
